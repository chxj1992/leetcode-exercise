...
class Solution:
    ...
    def buildTree(self, preorder: List[int], inorder: List[int]) -> TreeNode:
        def build_tree(preorder: List[int], inorder: List[int], offset: int):
            if not preorder:
                return None
            root = TreeNode(preorder[0])
            left_index = self.inorder_hash[preorder[0]] - offset
            root.left = build_tree(preorder[1:left_index + 1], inorder[0:left_index], offset)
            root.right = build_tree(preorder[left_index + 1:], inorder[left_index + 1:], offset + left_index + 1)
            return root

        self.inorder_hash = {x: i for i, x in enumerate(inorder)}
